...
Run the HMI anywhere — no Python installation needed on the target machine.

### Prerequisites

| Platform | X server required |
|---|---|
| **Linux** | Built-in X11 (no extras needed) |
| **macOS** | [XQuartz](https://www.xquartz.org/) |
| **Windows** | [VcXsrv](https://sourceforge.net/projects/vcxsrv/) or [X410](https://x410.app) |
...
1. Start **VcXsrv** (XLaunch → Multiple windows → Display 0 → tick **Disable access control**).
2. Run:

```powershell
# Mock mode (no PLC needed):
.\scripts\docker-run.ps1 -Mock

# Real ADS mode:
.\scripts\docker-run.ps1

# Rebuild image first, then run mock:
.\scripts\docker-run.ps1 -Mock -Build
```
...
# Mock mode:
./scripts/docker-run.sh --mock

# Real ADS mode:
./scripts/docker-run.sh

# Rebuild image first:
./scripts/docker-run.sh --mock --build
```
...
# Real ADS mode (set your PLC IP first):
export ADS_TARGET_IP=192.168.0.100
export ADS_TARGET_AMS=5.1.204.160.1.1
docker compose up
```
...
|---|---|---|
| `MOCK_ADS` | `0` | Set to `1` to use the built-in PLC simulator |
| `DISPLAY` | `:0` | X11 display to connect to |
| `ADS_TARGET_IP` | `192.168.0.1` | IP of the TwinCAT host |
| `ADS_TARGET_AMS` | `5.1.204.160.1.1` | AMS Net ID of the TC3 runtime |
