...
const avgEntries = reactive([{ price: '', qty: '' }, { price: '', qty: '' }])
...
function calcAvg() {
  try {
    error.value = ''
    const purchases = avgEntries.filter(e => e.price && e.qty).map(e => ({ price: +e.price, qty: +e.qty }))
...
const allocEntries = reactive([{ symbol: '', value: '' }, { symbol: '', value: '' }])
const allocResult = ref<Record<string, number | string> | null>(null)
function addAlloc() { allocEntries.push({ symbol: '', value: '' }) }
function calcAlloc() {
  try {
    error.value = ''
    const holdings = allocEntries.filter(e => e.symbol && e.value).map(e => ({ symbol: e.symbol.toUpperCase(), value: +e.value }))
      ...
      <div class="card p-5 space-y-4">
        ...
        <div v-for="(e, i) in avgEntries" :key="i" class="grid grid-cols-[1fr_1fr_auto] gap-2 items-end">
          ...
          <button v-if="avgEntries.length > 2" @click="removeEntry(i)"
            class="pb-2 text-red-400 hover:text-red-300 text-lg leading-none">×</button>
        </div>
