      ...
      <div v-for="entry in entries" :key="entry.id"
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-center gap-3 flex-wrap">
            <span :class="['badge border text-xs', catColor[entry.category] ?? 'text-gray-400 bg-gray-500/10 border-gray-500/20']">
              {{ entry.category }}
            </span>
            <span class="text-sm font-medium text-[var(--text-primary)]">{{ entry.label }}</span>
          </div>
          <span class="text-xs text-[var(--text-muted)] whitespace-nowrap flex-shrink-0">{{ relTime(entry.timestamp) }}</span>
        </div>
        <div class="mt-2 flex flex-wrap gap-x-4 gap-y-1">
          <span v-for="(v, k) in entry.results" :key="k" class="text-xs">
            <span class="text-[var(--text-muted)]">{{ k }}: </span>
            <span class="stat-value" :class="typeof v === 'number' && v < 0 ? 'text-loss' : 'text-[var(--text-primary)]'">
              {{ fmt(v) }}
            </span>
          </span>
        </div>
