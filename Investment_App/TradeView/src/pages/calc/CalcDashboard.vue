...
const categories = [
  { name: 'Price',     to: '/calc/price',    desc: 'Diff, % change, SL, TP, Pivot, SMA',    color: 'text-blue-400',    bg: 'bg-blue-500/10 border-blue-500/20' },
  { name: 'Returns',   to: '/calc/returns',  desc: 'P&L, CAGR, ROI, Dividend Yield, CI',     color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/20' },
  { name: 'Risk',      to: '/calc/risk',     desc: 'Position size, R/R, Sharpe, Volatility', color: 'text-orange-400',  bg: 'bg-orange-500/10 border-orange-500/20' },
  { name: 'Position',  to: '/calc/position', desc: 'Avg buy, Allocation, Lot size, P&L',     color: 'text-violet-400',  bg: 'bg-violet-500/10 border-violet-500/20' },
  { name: 'Options',   to: '/calc/options',  desc: 'Intrinsic value, Black-Scholes, Greeks', color: 'text-rose-400',    bg: 'bg-rose-500/10 border-rose-500/20' },
  { name: 'Percent',   to: '/calc/percent',  desc: 'Value % up / down — instant result',     color: 'text-cyan-400',    bg: 'bg-cyan-500/10 border-cyan-500/20' },
]
  ...
  <div class="space-y-8 max-w-5xl">
    ...
    <div>
      <div class="flex items-center justify-between mb-3">
        <h3 class="text-sm font-semibold text-[var(--text-secondary)] uppercase tracking-widest">Recent Calculations</h3>
        <button v-if="recent.length" @click="history.clear()"
          class="text-xs text-[var(--text-muted)] hover:text-red-400 transition-colors">Clear</button>
      </div>
      ...
      <div v-else class="card divide-y divide-[var(--surface-border)]">
        <div v-for="entry in recent" :key="entry.id"
          ...
          <div class="text-right">
            <span v-for="(v, k) in entry.results" :key="k"
